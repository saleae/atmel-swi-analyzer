...
AtmelSWIAnalyzer::AtmelSWIAnalyzer() : mSimulationInitilized( false )
{
        ...
        for( ;; )
        {
            try
            {
                ...
                if( Flag == SWIF_Command || Flag == SWIF_Transmit )
                {
                    ...
                    AddFrame( ( ByteSamples.end() - 2 )->first, ByteSamples.back().second, FrameChecksum, block.GetChecksum(),
                              block.CalcCRC() );
    ...
    else
    {
        ...
        for( param_cnt = 0; PacketParams[ param_cnt ].Name != NULL; ++param_cnt )
        {
                ...
                ( Count == param->ValidIfCount || param->ValidIfCount == 0 ) )
            {
                ...
                AddFrame( ByteSamples[ offset ].first, ByteSamples[ offset + param->Length - 1 ].second, FramePacketSegment, param_cnt,
                          data2 );
...
U32 AtmelSWIAnalyzer::GenerateSimulationData( U64 minimum_sample_index, U32 device_sample_rate,
